...
output "SSH-Login-ELKServer-command" {
  value = "ssh -i ${var.private_key_path} ${var.user}@${aws_instance.elk-server.public_dns}"
}

output "SSH-Login-client-logs-command" {
  value = "ssh -i ${var.private_key_path} ${var.user}@${aws_instance.elk-client.public_dns}"
}

output "Kibana-interface" {
  value = "http://${aws_instance.elk-server.public_ip}:90"
}
